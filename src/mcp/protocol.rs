...
//!
//! The native server maps the same tool outcomes into `rmcp` types, while the
//! Cloudflare adapter maps these values directly into Streamable HTTP events.
//! Keeping this module free of Tokio, sockets, and Worker types lets both
//! runtimes share one public contract and one execution path.

use serde_json::{Map, Value, json};

use crate::contract::{SERVER_NAME, SERVER_VERSION};

/// Latest stable protocol version advertised by the installed native SDK.
...
mod tests {
    ...
    #[test]
    fn calls_tools_with_the_native_result_shape() {
